...
x <- sprintf("%s/%s/Robjects", projDir, outDirBit)
x
#dir.create(x)
dir.exists(x)
```
...
```{r tenxLibStructureV3, out.width="80%"}
tmpFn <- sprintf("%s/Images/tenxLibStructureV3.png", projDirRel)
...
tmpFn <- sprintf("%s/Images/1_AAACCTGAGACTTTCG-1.rseqcGeneBodyCovCheck.txt.geneBodyCoverage.curves.png", projDirRel)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
```
...
# distribution of totReads, log2 scale
#summary(log2(ampDf$totReads))

# distribution of nUmis, log2 scale
...
The barcode rank plot shows the library sizes against their rank in decreasing order, for the first 10000 droplets only.
...
plot(log10(e.out$Total),
     -e.out$LogProb,
     col=cellColour, # ifelse(is.cell, "red", "black"),
...
```{r cellCall_emptyDrops_show}
sce.ed
```

Cell calling in cellranger v3 uses a method similar to emptyDrops() and a 'filtered matrix' is generated that only keeps droplets deemed to contain a cell (or more). We will load these filtered matrices now.
...
```{r dataSets_load, eval=poolBool, cache.lazy = FALSE}
# load data:
# a bit slow
...
# compare to taht for the first sample
...
```{r dataSets_writeRds, eval=TRUE, cache=FALSE}
...
```{r dataSets_readRds, eval=TRUE, cache=FALSE}
...
```{r amplification_2, eval=FALSE, include=FALSE}
# a bit slow
...
#if(!FALSE) # slow
if(!file.exists(tmpFn)) # slow, but needs running at least once
{
ampDf <- dd %>%
...
```{r ampDf_scatter_2, eval=FALSE, include=FALSE}
...
```{r ampDf_bin2d_2, eval=FALSE, include=FALSE}
sp2 <- ggplot(ampDf, aes(x=nUmis, y=totReads)) +
  geom_bin2d(bins = 50) +
  scale_fill_continuous(type = "viridis") +
  scale_x_continuous(trans='log10') +
...
```{r properties_totNbGenesDetected}
...
Summary of the sistribution of the number of genes detected per cell:
...
```{r qc_annot, eval=TRUE, cache=FALSE}
...
```{r qc_annot_writeRds, eval=TRUE, cache=FALSE}
...
```{r qc_annot_readRds, eval=TRUE, cache=FALSE}
...
```{r qc_addPerFeatureQC, eval=TRUE, cache=FALSE}
...
```{r qc_addPerCellQC, eval=TRUE, cache=FALSE}
...
```{r qc_addQc_writeRds, eval=TRUE, cache=FALSE}
...
```{r qc_addQc_readRds, eval=TRUE, cache=FALSE}
...
tmpFn <- sprintf("%s/%s/%s/qc_metricDistrib_plotColData2.png", projDirRel, outDirBit, qcPlotDirBit)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
```
...
```{r sce_qc_write, eval=TRUE, cache=FALSE}
...
```{r sce_qc_read, eval=TRUE, cache=FALSE}
...
tmpFn <- sprintf("%s/%s/%s/%s_sparsity.png", projDirRel, outDirBit, qcPlotDirBit, setName)
