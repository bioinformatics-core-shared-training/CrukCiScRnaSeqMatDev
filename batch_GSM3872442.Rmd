...
```{r, eval=FALSE, include=FALSE}
# Debug.
# Fix was adding rownames to data frame for cell_attr

pbmc_data <- counts(sce) 
class(pbmc_data)
dim(pbmc_data)
colnames(pbmc_data) <- colData(sce)$Barcode
colnames(pbmc_data) %>% head()

pbmc_data <- Read10X(data.dir = "filtered_gene_bc_matrices/hg19/")  # Seurat function to read in 10x count data
class(pbmc_data)
dim(pbmc_data)
colnames(pbmc_data) %>% head()
...
set.seed(44)
vst_out <- sctransform::vst(pbmc_data,
...
```{r}
print(sce.srt[["SCT"]]@misc$vst.out$model_str)
```
...
```{r}
sctransform::plot_model_pars(sce.srt[["SCT"]]@misc$vst.out)
```
