...
```{r preProc_allCells.knitr_options, echo=FALSE, results="hide", message=FALSE, dev="CairoPNG"}
cacheBool <- params$cacheBool
cacheLazyBool <- params$cacheLazyBool
require(knitr)
...
```{r libraries_allCells, include=FALSE}
library(scran)
library(scater)
...
```{r tenxLibStructureV3_allCells, out.width="80%"}
...
```{r example_dim_allCells}
dim(counts(sce.raw))
```

Features, with rowData():

```{r example_row_allCells}
head(rowData(sce.raw))
```

Samples, with colData():

```{r example_col_allCells}
...
```{r geneBodyCoverage_allCells, out.width="80%"}
...
```{r molecule_info_h5.Rds_allCells}
...
```{r ampDf_scatter_allCells, eval=FALSE}
# too slow
...
```{r cellCall_mixtools_density_plot_allCells, eval=FALSE}
...
```{r cellCall_barcodeRankPlot_initScale_allCells}
...
```{r cellCall_barcodeRankPlot_logScale_allCells}
...
```{r cellCall_barcodeRankPlot_logScale2_allCells}
...
The inflection is at `r inflection` UMIs (`r log10(inflection)` on the log10 scale).
`r sum(libSizeDf$nUmis > inflection)` droplets have at least these many UMIs and would thus contain one cell (or more).
...
```{r cellCall_cellrangerV1n2_allCells}
...
```{r cellCall_dropletUtils_allCells}
...
```{r cellCall_emptyDrops_run_allCells}
# a bit slow
...
```{r cellCall_emptyDrops_sigSummary_allCells}
...
```{r cellCall_emptyDrops_sigLimited_allCells}
...
```{r cellCall_emptyDrops_sigEmpty_allCells}
...
```{r cellCall_emptyDrops_isCell_allCells}
is.cell <- e.out$FDR <= 0.001
```

Number of cells detected: `r sum(is.cell, na.rm=TRUE)`.
...
```{r cellCall_emptyDrops_diagPlot_allCells}
# colour:
...
```{r cellCall_emptyDrops_ditch_allCells}
...
```{r cellCall_emptyDrops_show_allCells}
sce.ed
rm(sce.ed)
```
...
```{r dataSets_load_allCells, cache.lazy = cacheLazyBool, eval=poolBool}
# a bit slow
# load data:
...
Let's combine all `r length(sce.list)` samples into a single object.

We first check the feature lists are identical.

```{r dataSets_checkRows_allCells, eval=poolBool}
...
```{r dataSets_cbind_allCells, cache.lazy = cacheLazyBool, eval=poolBool}
sce <- sce.list[[1]]
...
```{r dataSets_addSplSheet_allCells, cache.lazy = cacheLazyBool, eval=poolBool}
...
```{r dataSets_writeRds_allCells, eval=poolBool}
# Write object to file
...
```{r dataSets_readRds_allCells, eval=!poolBool}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_postPool%s.Rds",
		 projDir, outDirBit, setSuf)
sce <- readRDS(tmpFn)
...
```{r amplification_2_allCells, eval=FALSE}
# a bit slow
...
```{r ampDf_scatter_2_allCells, eval=FALSE, include=FALSE}
# many points so very slow
sp <- ggplot(ampDf, aes(x=nUmis, y=totReads)) +
  geom_point() +
  scale_x_continuous(trans='log2') +
  scale_y_continuous(trans='log2')
...
```{r ampDf_bin2d_2_allCells, eval=FALSE}
sp2 <- ggplot(ampDf, aes(x=nUmis, y=totReads)) +
  geom_bin2d(bins = 50) +
  scale_fill_continuous(type = "viridis") +
  scale_x_continuous(trans='log2') +
...
```{r properties_totNbGenesDetected_allCells}
...
```{r properties_distribNbGenesDetected_allCells}
...
```{r corelLibSizePropCellExpress_write_allCells, eval=FALSE}
...
```{r corelLibSizePropCellExpress_write_allCells_2}
...
```{r corelLibSizePropCellExpress_show_allCells, out.width="80%", eval=FALSE}
getwd()
...
```{r scatter_mean_vs_sum_allCells, eval=FALSE, include = FALSE}
# randomly subset 1000 cells to speed plotting.
...
```{r relExpress_boxplot_allCells, cache.lazy = cacheLazyBool}
...
```{r qc_annot_writeRds_allCells, eval=biomartBool}
# slow
# Write object to file
##tmpFn <- sprintf("%s/%s/Robjects/sce_postBiomart%s.Rds",
##		 projDir, outDirBit, setSuf)
...
```{r qc_annot_readRds_allCells, cache.lazy = cacheLazyBool, eval=!biomartBool}
# Read object in:
##tmpFn <- sprintf("%s/%s/Robjects/sce_postBiomart%s.Rds",
##		 projDir, outDirBit, setSuf)
sce <- readRDS(tmpFn)
...
```{r qc_addPerFeatureQC_allCells, cache.lazy = cacheLazyBool, eval=addQcBool}
# long
# for genes
...
```{r qc_addPerCellQC_allCells, cache.lazy = cacheLazyBool, eval=addQcBool}
# for cells
...
```{r qc_addQc_writeRds_allCells, eval=addQcBool}
# Write object to file
...
```{r qc_addQc_readRds_allCells, eval=!addQcBool}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_postAddQc%s.Rds",
		 projDir, outDirBit, setSuf)
sce <- readRDS(tmpFn)
...
```{r qc_metricDistrib_twoHist_allCells}
par(mfrow=c(1, 2))
...
```{r qc_metricDistrib_plotColData_show_allCells, out.width="80%", eval=FALSE}
...
```{r plotColData_libSize_AllCells}
    plotColData(sce, x="block", y="sum",
        other_fields="setName") + #facet_wrap(~setName) + 
        scale_y_log10() + ggtitle("Total count")
```
...
```{r plotColData_nbGenes_AllCells}
    plotColData(sce, x="block", y="detected", 
        other_fields="setName") + #facet_wrap(~setName) + 
        scale_y_log10() + ggtitle("Detected features")
```
...
```{r plotColData_mitoContent_AllCells}
    plotColData(sce, x="block", y="subsets_Mito_percent", 
        other_fields="setName") + # facet_wrap(~setName) +
        ggtitle("Mito percent")
```
...
```{r scatter_detected_vs_sum_colBy_sourceName_allCells}
...
```{r scatter_detected_vs_sum_colBy_mitoContent_allCells}
...
```{r libSize_qcLib2_thresholds_allCells}
...
```{r libSize_qcNexprs2_thresholds_allCells}
...
```{r libSize_qcMito2_thresholds_allCells}
attr(qc.mito2, "thresholds")
```

### Summary

```{r adapThres_summary_allCells}
...
```{r adapThres_quickPerCellQC_allCells}
...
```{r quickPerCellQC_batch_compute_allCells, cache.lazy = cacheLazyBool}
...
```{r quickPerCellQC_batch_table_allCells}
table(reasons$discard, batch.reasons$discard)
```

```{r quickPerCellQC_batch_plot_allCells, eval=runAll}
...
```{r qc_metricDistrib_plotColDataBatch_show_allCells, out.width="80%", eval=FALSE}
...
```{r plotColData_libSize_colBy_discard_allCells}
plotColData(sce, x="block", y="sum", colour_by="discard",
        other_fields="setName") +
	#facet_wrap(~setName) + 
        scale_y_log10() + ggtitle("Total count")
...
```{r plotColData_nbGenes_colBy_discard_allCells}
    plotColData(sce, x="block", y="detected", colour_by="discard", 
        other_fields="setName") +
	#facet_wrap(~setName) + 
        scale_y_log10() + ggtitle("Detected features")
...
```{r plotColData_nbGenes_colBy_discard_facetBy_discard_allCells}
plotColData(sce, x="block", y="detected", colour_by="discard", 
        other_fields="setName") +
...
```{r plotColData_mitoContent_colBy_discard_AllCells}
    plotColData(sce, x="block", y="subsets_Mito_percent", 
        colour_by="discard", other_fields="setName") + 
        #facet_wrap(~setName) +
  	ggtitle("Mito percent")
...
```{r scatter_mitoContent_vs_detected_colBy_discard_AllCells}
sp <- ggplot(data.frame(colData(sce)),
...
```{r scatter_detected_vs_mitoContent_colBy_discard_allCells}
sp <- ggplot(data.frame(colData(sce)), aes(x=detected, y=subsets_Mito_percent, col=discard)) +
  geom_point(size = 0.05, alpha = 0.2)
sp + facet_wrap(~source_name)
```

```{r scatter_detected_vs_mitoContent_allCells, eval=FALSE}
...
```{r scatter_detected_vs_mitoContent_allCells_2}
sp <- ggplot(data.frame(colData(sce)), aes(x=detected, y=subsets_Mito_percent)) +
  geom_point(size = 0.05, alpha = 0.2) +
  geom_density_2d(size = 0.5, colour = "blue")
ggExtra::ggMarginal(sp)
...
```{r qc_poorBatch_detected_compute_allCells, eval=TRUE}
# compute
...
```{r plotColData_nbGenes_woBlocking_allCells}
...
```{r plotColData_nbGenes_wiBlocking_allCells}
# plots - with blocking
...
```{r qc_poorBatch_mitoContent_compute_allCells}
...
```{r plotColData_mitoContent_woBlocking_allCells}
# plots - without blocking
...
```{r plotColData_mitoContent_wiBlocking_allCells}
# plots - with blocking
...
```{r qc_poorBatch_detected_list_allCells}
# names
...
```{r qc_poorBatch_mito_list_allCells}
# names
...
```{r summary_multiOutlier_allCells}
summary(multi.outlier)
```

Compare with previous filtering:

```{r table_discard_multiOutlier_allCells, eval=TRUE}
...
```{r PCA_coldata_colBy_outlier_allCells, cache.lazy = cacheLazyBool}
...
```{r plotColData_mitoContent_vs_sum_colBy_discard_AllCells}
plotColData(sce, x="sum", y="subsets_Mito_percent", colour_by="discard")
```

```{r plotColData_mitoContent_vs_sum_colBy_discard_AllCells_2}
sp <- ggplot(data.frame(colData(sce)),
...
```{r plotColData_mitoContent_vs_sum_colBy_discard_AllCells_3}
sp + facet_wrap(~source_name)
```
...
```{r sce_qc_ditch_allCells, cache.lazy = cacheLazyBool}
scePreQc <- sce
...
```{r sce_qc_write_allCells, eval=runAll}
# Write object to file
...
```{r sce_qc_read_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_postQc%s.Rds",
		 projDir, outDirBit, setSuf)
...
```{r novelty_scat_allCells, cache.lazy = cacheLazyBool, eval=TRUE}
p <- colData(sce) %>%
  data.frame() %>%
...
```{r sce_nz_allCells, cache.lazy = cacheLazyBool, eval=runAll}
...
```{r sce_nz_write_allCells, eval=runAll}
# Write object to file
...
```{r sce_nz_read_allCells, cache.lazy = cacheLazyBool, eval=!runAll}
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/sce_nz%s.Rds",
		 projDir, outDirBit, setSuf)
...
Number of genes `r nrow(sce.nz)`.

Number of cells `r ncol(sce.nz)`.
...
```{r sparsity_compute_allCells, cache.lazy = cacheLazyBool, eval=runAll}
# compute - SLOW
...
```{r sparsity_readRds_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_nz_sparsityCellGene%s.Rds",
		 projDir, outDirBit, setSuf)
...
```{r sparsity_plot_allCells, eval=runAll}
# plot
...
```{r sparsity_show_allCells, out.width="80%"}
...
```{r sparsity_filter_allCells, eval=TRUE}
# filter
...
```{r table_sparseGenes_allCells}
table(sparse.genes)
```

Number of cells removed:

```{r table_sparseGenes_mitoCells_allCells}
table(sparse.cells, mito.cells)
```

```{r sparsity_ditch_allCells, cache.lazy = cacheLazyBool, eval=runAll}
...
```{r sparsity_filter_write_allCells, eval=runAll}
# Write object to file
...
```{r sparsity_filter_read_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setSuf)
#print("DEV"); print(getwd()); print(tmpFn)
...
```{r compare_filters_1_allCells}
table(scePreQc$discard, (sparse.cells | mito.cells))
```

```{r rm_scePreQc_allCells}
rm(scePreQc)
```
...
```{r sparsity_split_allCells, cache.lazy = cacheLazyBool, eval=TRUE}
...
```{r Caron_sparsity_copy_allCells, eval=TRUE}
setName <- "caron"
sce.x <- sce.nz.caron # for code re-use
rm(sce.nz.caron)
```

```{r Caron_sparsity_compute_allCells, eval=runAll}
# compute - SLOW
...
```{r Caron_sparsity_readRds_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_sparsityCellGene%s.Rds",
		 projDir, outDirBit, setName, setSuf)
tmpList <- readRDS(tmpFn)
...
```{r Caron_sparsity_plot_allCells, eval=runAll}
# plot
...
```{r Caron_sparsity_show_allCells, out.width="80%"}
...
```{r Caron_sparsity_filter_allCells, eval=TRUE}
# filter
sparse.cells <- cell_sparsity > 0.99
...
```{r Caron_sparsity_ditch_allCells, eval=runAll}
# remove cells from the SCE object that are poor quality
# remove the sparse genes, then re-set the counts and row data accordingly
...
```{r Caron_sparsity_filter_write_allCells, eval=runAll}
# Write object to file
...
```{r Caron_sparsity_filter_read_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setName, setSuf)
...
```{r Hca_sparsity_copy_allCells, cache.lazy = cacheLazyBool, eval=TRUE}
setName <- "hca"
sce.x <- sce.nz.hca # for code re-use
rm(sce.nz.hca)
```

```{r Hca_sparsity_compute_allCells, cache.lazy = cacheLazyBool, eval=runAll}
# compute - SLOW
cell_sparsity <- apply(counts(sce.x) == 0, 2, sum)/nrow(counts(sce.x))
gene_sparsity <- apply(counts(sce.x) == 0, 1, sum)/ncol(counts(sce.x))
...
```{r Hca_sparsity_readRds_allCells, eval=runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_sparsityCellGene%s.Rds",
		 projDir, outDirBit, setName, setSuf)
tmpList <- readRDS(tmpFn)
...
```{r Hca_sparsity_show_allCells, out.width="80%"}
...
```{r Hca_sparsity_filter_allCells, eval=runAll}
# filter
sparse.cells <- cell_sparsity > 0.99
mito.cells <- sce.x$subsets_Mito_percent > 20
...
```{r Hca_sparsity_ditch_allCells, cache.lazy = cacheLazyBool, eval=runAll}
# remove cells from the SCE object that are poor quality
# remove the sparse genes, then re-set the counts and row data accordingly
cols.meta <- colData(sce.x)
rows.meta <- rowData(sce.x)
...
```{r Hca_sparsity_filter_write_allCells, cache.lazy = cacheLazyBool, eval=runAll}
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setName, setSuf)
saveRDS(sce.x, tmpFn)
...
```{r Hca_sparsity_filter_read_allCells, cache.lazy = cacheLazyBool, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setName, setSuf)
...
```{r Hca_downsample_allCells, eval=runAll}
sce.nz.hca
