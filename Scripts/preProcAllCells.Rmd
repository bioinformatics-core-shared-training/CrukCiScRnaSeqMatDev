...
```{r preProc_allCells.knitr_options, echo=FALSE, results="hide", message=FALSE}
...
# Quality Control {#PreProcAllCellsTop}
...
```{r dataSets_load_allCells, cache.lazy = FALSE, eval=poolBool}
# a bit slow
# load data:
...
```{r dataSets_cbind_allCells, cache.lazy = FALSE, eval=poolBool}
sce <- sce.list[[1]]
...
```{r dataSets_addSplSheet_allCells, cache.lazy = FALSE, eval=poolBool}
...
tmpFn <- sprintf("%s/%s/%s/corelLibSizePropCellExpress%s_2.png",
		 projDir, outDirBit, qcPlotDirBit, setSuf)
tmpFn
#ggsave(ggExtra::ggMarginal(sp), file=tmpFn)
rm(tmpFn)
```
...
```{r relExpress_boxplot_allCells, cache.lazy = FALSE}
...
```{r qc_annot_allCells, eval=biomartBool}
...
```{r qc_annot_readRds_allCells, cache.lazy = FALSE, eval=!biomartBool}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_postBiomart%s.Rds",
		 projDir, outDirBit, setSuf)
sce <- readRDS(tmpFn)
...
```{r qc_addPerFeatureQC_allCells, cache.lazy = FALSE, eval=addQcBool}
# long
# for genes
...
```{r qc_addPerCellQC_allCells, cache.lazy = FALSE, eval=addQcBool}
# for cells
...
```{r qc_metricDistrib_prepPlot_allCells, cache.lazy = FALSE}
...
```{r quickPerCellQC_batch_compute_allCells, cache.lazy = FALSE}
...
```{r PCA_coldata_colBy_outlier_allCells, cache.lazy = FALSE}
...
```{r sce_qc_ditch_allCells, cache.lazy = FALSE}
scePreQc <- sce
...
```{r novelty_scat_allCells, cache.lazy = FALSE, eval=TRUE}
p <- colData(sce) %>%
  data.frame() %>%
...
```{r sce_nz_read_allCells, cache.lazy = FALSE, eval=!runAll}
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/sce_nz%s.Rds",
		 projDir, outDirBit, setSuf)
...
```{r sparsity_compute_allCells, eval=runAll}
# compute - SLOW
...
```{r sparsity_ditch_allCells, eval=runAll}
...
```{r sparsity_split_allCells, cache.lazy = FALSE, eval=TRUE}
...
```{r Hca_sparsity_copy_allCells, cache.lazy = FALSE, eval=TRUE}
setName <- "hca"
sce.x <- sce.nz.hca # for code re-use
rm(sce.nz.hca)
```

```{r Hca_sparsity_compute_allCells, eval=runAll}
# compute - SLOW
cell_sparsity <- apply(counts(sce.x) == 0, 2, sum)/nrow(counts(sce.x))
gene_sparsity <- apply(counts(sce.x) == 0, 1, sum)/ncol(counts(sce.x))
...
```{r Hca_sparsity_ditch_allCells, eval=runAll}
# remove cells from the SCE object that are poor quality
# remove the sparse genes, then re-set the counts and row data accordingly
cols.meta <- colData(sce.x)
rows.meta <- rowData(sce.x)
...
```{r Hca_sparsity_filter_write_allCells, eval=runAll}
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setName, setSuf)
saveRDS(sce.x, tmpFn)
...
```{r Hca_sparsity_filter_read_allCells, cache.lazy = FALSE, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds",
		 projDir, outDirBit, setName, setSuf)
