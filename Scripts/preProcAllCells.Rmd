...
```{r dataSets_load_allCells, eval=poolBool}
# a bit slow
# load data:
...
```{r dataSets_cbind_allCells, eval=poolBool}
sce <- sce.list[[1]]
...
```{r dataSets_addSplSheet_allCells, eval=poolBool}
...
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/sce_postPool%s.Rds", projDir, outDirBit, setSuf)
saveRDS(sce, tmpFn)
```
...
```{r qc_annot_readRds_allCells, eval=!biomartBool}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/sce_postBiomart%s.Rds",
		 projDir, outDirBit, setSuf)
sce <- readRDS(tmpFn)
...
```{r qc_addPerFeatureQC_allCells, eval=addQcBool}
# long
# for genes
...
```{r qc_addPerCellQC_allCells, eval=addQcBool}
# for cells
...
```{r qc_metricDistrib_prepPlot_allCells}
...
```{r quickPerCellQC_batch_compute_allCells}
...
```{r PCA_coldata_colBy_outlier_allCells}
...
```{r novelty_scat_allCells, eval=TRUE}
p <- colData(sce) %>%
  data.frame() %>%
...
```{r sce_nz_read_allCells, eval=!runAll}
# Write object to file
tmpFn <- sprintf("%s/%s/Robjects/sce_nz%s.Rds",
		 projDir, outDirBit, setSuf)
...
```{r sparsity_split_allCells, eval=TRUE}
...
```{r Hca_sparsity_copy_allCells, eval=TRUE}
setName <- "hca"
sce.x <- sce.nz.hca
```
...
# write outcome to file for later use
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_sparsityCellGene%s.Rds", projDir, outDirBit, setName, setSuf)
saveRDS(list("colData" = colData(sce.x),
	     "rowData" = rowData(sce.x)),
	tmpFn)
```
...
```{r Hca_sparsity_filter_read_allCells, eval=!runAll}
# Read object in:
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postQc%s.Rds", projDir, outDirBit, setName, setSuf)
