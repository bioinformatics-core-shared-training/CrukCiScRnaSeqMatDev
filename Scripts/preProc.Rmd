...
We will load the data for the **SRR9264343**. 
...
example a `dgCMatrix` object. Transforming the raw count matrix to a sparse 
matrix object will significantly reduce the memory required for processing the
data.
...
percentage of mitochondrial genes, to this data frame.
...
The plot below show the average coverage (y-axis) along the body of genes
(x-axis).

```{r geneBodyCoverage, out.width="80%"}
knitr::include_graphics("Images/geneBodyCoverage.curves.png", auto_pdf = TRUE)
```

### Amplification rate

We will use the information stored in the 'molecule info' file to count the
number of UMI and reads for each gene in each cell.

```{r molecule_info_h5.Rds}
mol.info <- readRDS("Course_Materials/CellRanger_Outputs/SRR9264343/outs/molecule_info_h5.Rds")
```
...
vector will be used as the sample names in the Single Cell Experimen object.
...
sce <- read10xCounts(list_of_files)
```

Currently, the droplet annotation in `colData` slot of the `sce` object has two
columns: "Sample" and "Barcode".

```{r}
colData(sce)
```

The "latter"Barcode" column contains the cell/droplet barcode and comprises the
actual sequence and a 'group ID', e.g. AAACCTGAGAAACCAT-1. The 'group ID' helps
distinguish cells from different samples that have identical barcode sequences.
As each sample was processed separately with CellRanger, the group ID is set to
1 in all data sets. To pool these data sets we need to change group IDs so cell
barcodes are unique across all samples.
...
colData(sce) <- colData(sce) %>% 
    as.data.frame() %>%
    mutate(SampleNum = as.numeric(as.factor(Sample))) %>%
    mutate(Barcode = str_replace(Barcode, "1$", as.character(SampleNum))) %>%
