...
The meta data for the Caron data set can be obtained by the SRA Run Selector:

https://www.ncbi.nlm.nih.gov/Traces/study/?acc=PRJNA548203&o=acc_s%3Aa

The meta data for the Hca...

```{r samplesheet_preProc}
samplesheet.cb <- read_csv("Course_Materials/Data/CaronBourque2020/SraRunTable.txt")
samplesheet.hca <- read_csv("Course_Materials/Data/Hca/accList_Hca.txt",
                            col_names = "Run") %>%
    mutate(`Sample Name`=Run, source_name = "ABMMC")
samplesheet <- bind_rows(samplesheet.cb, samplesheet.hca) %>%
    select(Run, SampleName=`Sample Name`, source_name)
```
