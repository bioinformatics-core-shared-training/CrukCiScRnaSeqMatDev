...
Fewer cells have been discarded, this is largely due to the change in the 
theshold for the library size. As we can see from the violin plots
the ABMMC samples from the HCA have a radically different library size
distribution to the Caron samples, with all cells having > 1000 UMIs. When we
applied the adaptive filters across all samples, these two samples caused the 
MADs to be distorted and resulted in a threshold that was inappropriately high 
for the Caron samples.
