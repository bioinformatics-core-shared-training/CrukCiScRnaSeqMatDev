...
```{r}
isBM1 <- sce$Sample == "MantonBM1"
sce.BM1 <- subset(sce, select = isBM1)
```
...
can then use `isOutlier` to select outliers based on this metric. One way to do
this is to use the function `adjOutlyingness` from the `robustbase` package. This 
function computes the "outlyingness" for each droplet.
...
Another approach is to perform a principal components analysis (PCA) on the table
of metrics, apply `adjOutlyingness` to the metrics table and use this to detect 
outliers. The `scater` function `runColDataPCA` can be used to perform the PCA 
and detect outliers. We'll need to add a couple of columns to the `colData` for
the log10 metrics first.
