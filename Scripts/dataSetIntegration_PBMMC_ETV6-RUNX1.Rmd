...
# Data integration - PBMMC + ETV6-RUNX {#dataSetIntegration_PBMMC_ETV6-RUNX1Top}
...
nbPcToComp <- 50
#splSetToGet <- params$splSetToGet # params may not be read in if knitting book.
splSetToGet <- "PRE-T" # params may not be read in if knitting book.
```
...
```{r dataSetIntegration_PBMMC_ETV6-RUNX1_setup, include=FALSE, echo=FALSE, dev="CairoPNG"}
...
p <- plotTSNE(uncorrected, colour_by="batch")
```
...
```{r, fig.height=figSize*1/2, fig.width=figSize}
...
Let's write the corresponding SCE object.
...
The `corrected` matrix in the `reducedDims()` contains the low-dimensional corrected coordinates for all cells, which we will use in place of the PCs in our downstream analyses (`r nrow(reducedDim(mnn.out, "corrected"))` cells and `r ncol(reducedDim(mnn.out, "corrected"))` PCs).

```{r}
dim(reducedDim(mnn.out, "corrected"))
```

A `reconstructed` matrix in the `assays()` contains the corrected expression values for each gene in each cell, obtained by projecting the low-dimensional coordinates in corrected back into gene expression space (`r nrow(assay(mnn.out, "reconstructed"))` genes and `r ncol(assay(mnn.out, "reconstructed"))` cells). We do not recommend using this for anything other than visualization.
...
Cluster size and cell contribution by tissue type, with clusters sorted by size:
...
tab <- table(paste("after", clusters.mnn[rescaled2$batch=="GSM3872434"]),
    paste("before", colLabels(rescaled[[1]])))
heat1 <- pheatmap(log10(tab+10), cluster_row=FALSE, cluster_col=FALSE,
    main="GSM3872434 comparison", silent=TRUE)

# For the second batch.
tab <- table(paste("after", clusters.mnn[rescaled2$batch=="GSM3872435"]),
    paste("before", colLabels(rescaled[[2]])))
heat2 <- pheatmap(log10(tab+10), cluster_row=FALSE, cluster_col=FALSE,
    main="GSM3872435 comparison", silent=TRUE)
...
splIdx <- 1
tab <- table(
  paste("after", clusters.mnn[rescaled2$batch==splVec[splIdx]]),
...
```{r, eval=FALSE, include=FALSE}
# DEV
gridExtra::grid.arrange(pNoLeg,
             gridExtra::arrangeGrob(pEdgeCol.leg, pEdgeAlpha.leg, pColour.leg, pSize.leg, ncol=2),
             ncol=1)
```
...
```{r, fig.height=figSize*length(treeList)*2/3, fig.width=figSize}
...
treeList[[1]] +
  ...
  #guides(edge_colour = TRUE, edge_alpha = TRUE) +
  guides(size = FALSE) +
  guides(colour = FALSE) 
```
...
```{r, fig.height=figSize, fig.width=figSize}
gridExtra::grid.arrange(grobs = treeList,
      ncol=2
)
```
...
```{r, fig.height=figSize*1/2, fig.width=figSize}
plotTSNE(mnn.out2, colour_by="source_name") + facet_wrap(~colData(mnn.out2)$source_name)
```
...
```{r}
p + facet_wrap(~colData(mnn.out2)$batch)
```
...
p1 <- ggplot(data=tmpMatDf, aes(x=clusters,y=Freq, fill=batch)) +
	theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
	geom_col()
p2 <- ggplot(data=tmpMat, aes(x=clusters, fill=Sample.Name)) +
  geom_bar(position = "fill") +
	theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
  scale_y_continuous(labels = scales::percent)
gridExtra::grid.arrange(p1, p2)
...
p <- ggplot(data=norm2, aes(x=clusters,y=Freq, fill=source_name)) +
	theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
	geom_col()
p
p <- ggplot(data=norm2, aes(x=clusters,y=Freq, fill=Sample.Name)) +
	theme(axis.text.x=element_text(angle = 90, hjust = 0)) +
	geom_col()
p
p + facet_wrap(~source_name)
```
...
```{r}
tab.mnn <- table(Cluster=clusters.mnn, Batch=as.character(mnn.out$source_name))
tab.mnn
...
Have threshold for proportion of PBMMC cells, say 50% and filter cluster with prop higher.
...
p + facet_wrap(~mnn.out$propLt080)
```

Same as above but with propLt050
...
p + facet_wrap(~mnn.out$propLt050)
...
print(p + facet_wrap(~mnn.out$propLt050))
}
```

<!-- large cluster with moderate PBMMC proportion -->

Check for clusters with large number of cancer cells and high proportion of PBMMC cells.

```{r, eval=FALSE}
# DEV, hardcoded cluster 2
normNoLog <- normNoLog %>% tibble::rownames_to_column("cluster")
normNoLog$cluster <- paste0("c", normNoLog$cluster)
normNoLog %>% filter(sum > 1000 & prop > 0.5) # ok with 3000 cells per sample

normNoLog$tmpCluBool <- ifelse(normNoLog$cluster=="c2", TRUE, FALSE)
...
p + facet_wrap(~mnn.out$tmpCluBool)
```

```{r, eval=FALSE}
# DEV, hardcoded cluster 2
genex <- rownames(demo)[5]
p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed")
p <- p + ggtitle(
			paste("cluster", cluToGet, genex,
			rowData(uncorrected)[genex,"Symbol"])
...
p + facet_wrap(mnn.out$source_name~mnn.out$tmpCluBool)
```
...
- proportion of cancer cells in cluster lower than 10% of cells of that sample type
...
propLtDf <- normNoLog %>%
  filter(prop < thdPropPbmmc | !!sym(otherSplType) > thdSize)
...
```{r sessionInfo_dataSetIntegration_PBMMC_ETV6-RUNX1}
sessionInfo()
```
