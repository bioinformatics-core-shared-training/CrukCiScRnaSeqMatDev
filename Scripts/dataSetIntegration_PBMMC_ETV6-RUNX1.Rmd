...
# Data integration - PBMMC and ETV6-RUNX {#dsi`r anaStg`-PBMMC-ETV6-RUNX1Top}

```{r variables_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r setup_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, include=FALSE, echo=FALSE, dev="CairoPNG"}
...
objects in a `large list`. Each sample has been subsets to 500 cells per sample for demonstration purposes.
...
We load the samplesheet with all the metadata from SRA:

```{r subset_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
sampleSheet <- read_csv("../Course_Materials/Data/CaronBourque2020/SraRunTable.txt")
```
...
```{r stdWkf_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
#--- normalization ---#
# use logNormCounts()
...
```{r prep_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r universe_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r multiBatchNorm_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r combineVar_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r diagPrep_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r diagClu_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r diagPlotDraw_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=2/3*figSize, fig.width=figSize}
...
```{r diagTsneDraw_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
set.seed(1111001)
...
```{r diagTsnePlot1_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
p
```

```{r diagTsnePlotSplit_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r diagWriteRds_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
splSetToGet2 <- gsub(",", "_", splSetToGet)
# save object?
...
```{r linReg_rescaleBatches_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r linReg_diagClu_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r linReg_diagPlotDraw_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=2/3*figSize, fig.width=figSize}
tmpMatTab <- table(tmpMat)
sortVecNames <- tmpMatTab %>% rowSums %>% sort(decreasing=TRUE) %>% names
tmpMat$clusters <- factor(tmpMat$clusters, levels=sortVecNames)
tmpMatDf <- tmpMatTab[sortVecNames,] %>% data.frame()
...
```{r linReg_diagTsneDraw_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
rescaled2 <- runTSNE(rescaled2, dimred="PCA")
rescaled2$batch <- factor(rescaled2$batch)
p <- plotTSNE(rescaled2, colour_by="batch")
```

```{r linReg_diagTsnePlot1_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize, fig.width=figSize}
p
```

```{r linReg_diagTsnePlotSplit_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
p + facet_wrap(~uncorrected$source_name)
```
...
```{r fastmnnGlance_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnnRbindColData_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnn_clu_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnn_diagPlotDrawSampleName_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=2/3*figSize, fig.width=figSize}
...
```{r fastmnn_diagPlotDrawSampleType_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=2/3*figSize, fig.width=figSize}
mnn.out$source_name <- uncorrected$source_name # cell order is maintained by scran functions
...
```{r fastmnn_diagVarComp_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnn_diagTsnePlot1_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
set.seed(0010101010)
...
```{r fastmnn_diagTsnePlot2_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r fastmnn_diagLostVar_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnn_diagTsneClu_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r fastmnn_diagTsneCellTypeMarker_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
genesToShow <- c(
		 "CD79A", # CD79A 	B ***
		 "CST3", # CST3 	monocytes ***
		 "CD3D", # CD3D 	 T cells ***
...
```{r uncorr_diagTsneB_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
# B cells
genex <- ensToShow[1]
...
```{r uncorr_diagTsneCellTypeMarkerShow_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r biolHet_comp_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, warning=FALSE}
...
```{r biolHet_plotDraw_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, warning=FALSE}
...
```{r biolHet_plotShowHeatmap_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r biolHet_plotShowTree_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*length(treeList)*2/3, fig.width=figSize}
...
```{r biolHet_plotDrawTree2_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r biolHet_plotShowTree2_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize, fig.width=figSize}
gridExtra::grid.arrange(grobs = treeList,
      ncol=2
)
```
...
```{r coasProb_plot_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, warning=FALSE}
...
```{r randIndex_comp_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, warning=FALSE}
...
```{r randIndex_plot_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r consis_comp_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r consis_fastmnn_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
set.seed(1000110)
...
```{r consis_plotTsne_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
plotTSNE(mnn.out2, colour_by="batch", shape_by="source_name")
```

```{r consis_plotTsneBySampleType_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
plotTSNE(mnn.out2, colour_by="source_name") +
	facet_wrap(~colData(mnn.out2)$source_name, ncol=2)
```

```{r consis_plotTsneByBatch_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize, fig.width=figSize}
plotTSNE(mnn.out2, colour_by="batch", shape_by="source_name") +
...
```{r consis_plotTsneCluBySample_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
plotList <- vector(mode = "list", length = length(splVec))
...
```{r consis_uncorr_findMarkers_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r consis_uncorr_topGene_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r consis_uncorr_tsneWoEncour_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r consis_uncorr_tsneWiEncour_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
#genex <- rownames(demo)[1]
...
```{r consis_mnnoutSaveRds_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r idPbmmcClu_barPlotDescSize_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
mnn.out$source_name <- uncorrected$source_name # cell order is maintained by scran functions
tmpMat <- data.frame("clusters"=clusters.mnn, "batch"=mnn.out$source_name)
tmpMatTab <- table(tmpMat)
sortVecNames <- tmpMatTab %>% rowSums %>% sort(decreasing=TRUE) %>% names
...
```{r idPbmmcClu_barPlotDescPropPbmmc_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r idPbmmcClu_contingTabCluByBatch_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
...
```{r idPbmmcClu_contingTabCluByType_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
tab.mnn <- table(Cluster=clusters.mnn,
		 Batch=as.character(mnn.out$source_name))
##tab.mnn
...
```{r idPbmmcClu_barPlotPropPbmmcThd050_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd050_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcThd080_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd080_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize, fig.width=figSize}
# cluster:
p <- plotTSNE(mnn.out, colour_by = "cluster", by_exprs_values="reconstructed")
p <- p + ggtitle("clusters")
...
```{r idPbmmcClu_barPlotPropPbmmcTsneThd050CellTypeMarkers_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize, eval=!FALSE}
for (genex in ensToShow)
{
	p <- plotTSNE(mnn.out, colour_by = genex, by_exprs_values="reconstructed") +
...
```{r idPbmmcClu_select_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=figSize*1/2, fig.width=figSize}
...
```{r idPbmmcClu_barPlotPropPbmmcTsneSelec_dsi{{setSuf}}_PBMMC_ETV6-RUNX1, fig.height=0.5*figSize, fig.width=figSize, eval=!FALSE}
# cluster:
p <- plotTSNE(mnn.out, colour_by = "cluster", by_exprs_values="reconstructed")
p <- p + ggtitle("clusters")
...
```{r idPbmmcClu_normNoLogSaveRds_dsi{{setSuf}}_PBMMC_ETV6-RUNX1}
splSetToGet2 <- gsub(",", "_", splSetToGet)
# save object?
