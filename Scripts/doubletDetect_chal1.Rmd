...
subtitle: 'Doublet detection - Challenge'
...
Using the method based the simulation of doublets from the single-cell expression profiles, we will identify droplets that may include more than one cell in one of the adult BMMC samples as they each comprise about 20,000 cells are are therefore likely to include doublets.

Note: the code used to illustrate the approach with one of the cancer sample uses cluster to help visualise the doublet score. We will also define cluster here, changing one parameter to account for the larger number of cell in the adult BMMC sample: to define cluster with buildSNNGraph() we will set the number of nearest neighbors to consider during graph construction (k) to 50, instead of 25.

The computation may last about 25 min.
...
```{r doubletDetect_chal1_setup, include=FALSE, echo=FALSE}
...
* scater, for count normalisation 
* scran, for UMI count normalisation and doublet detection 
<!-- * BiocSingular, for faster PCA -->
...
# hca_sce_nz_postQc_allCells.Rds
```

Input file: `r tmpFn`.

```{r}
if(!file.exists(tmpFn)) { knitr::knit_exit() }
sce <- readRDS(tmpFn)
```
...
We will get data for one sample, e.g. `r splNames[1]`:

```{r}
sceOrig <- sce
sce <- sceOrig[,sce$Sample.Name == splNames[1]]
sce <- sce[,sample(ncol(sce), 2000)] # for dev
```
...
#--- clustering ---#
snn.gr <- buildSNNGraph(sce, use.dimred="PCA", k=50)
...
We also plot the t-SNE showing clusters, which are not used for the doublet detection by simulation used here, but help visualize cells with different expression profiles.
...
Let us run `computeDoubletDensity()` and display moments of the distribution of the doublet scores returned:
