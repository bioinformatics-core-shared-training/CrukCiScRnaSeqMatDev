...
```{r norm_GSM3872434.knitr_options, echo=FALSE, results="hide", message=FALSE, dev="CairoPNG"}
...
```{r calcNormFactors_comp_norm_GSM3872434_allCells}
...
```{r librarySizeFactors_comp_norm_GSM3872434_allCells}
...
```{r librarySizeFactors_hist_norm_GSM3872434_allCells}
...
```{r load_quickClus_norm_GSM3872434_allCells}
# read from file
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_quickClus%s.Rds",
		 projDir, outDirBit, setName, setSuf)
clust <- readRDS(tmpFn)
...
```{r load_deconvSf_norm_GSM3872434_allCells}
# read from file
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_deconvSf%s.Rds",
		 projDir, outDirBit, setName, setSuf)
...
```{r scatter_deconvSf_libSf_plot_norm_GSM3872434_allCells, eval=FALSE, include=FALSE}
plot(lib.sf,
     deconv.sf,
...
```{r scatter_deconvSf_libSf_colBy_sourceName_plot_norm_GSM3872434_allCells}
...
```{r scatter_deconvSf_libSf_colBy_more_norm_GSM3872434_allCells, eval=FALSE, include=FALSE}
...
```{r scatter_deconvSf_libSf_colBy_cellSpars_norm_GSM3872434_allCells}
...
```{r gene_attr_sct_GSM3872434_allCells}
dim(gene_attr)
head(gene_attr)
```

```{r cell_attr_sct_GSM3872434_allCells}
dim(cell_attr)
head(cell_attr)
```
...
```{r scatter_nGene_nUmi_sct_GSM3872434_allCells, eval=FALSE, include=FALSE}
...
```{r plot_model_2_sct_GSM3872434_allCells}
sctransform::plot_model(vst_out,
                        counts,
                        c('ENSG00000087086'),
                        plot_residual = TRUE,
...
```{r table_write_sct_GSM3872434_allCells, eval=TRUE}
# write to file
...
```{r sce_write_sct_GSM3872434_5hCellPerSpl, eval=TRUE}
# write to file
...
```{r comp_pca_logRaw_GSM3872434_allCells}
typeNorm <- "logRaw"
...
```{r plot_pca_logRaw_GSM3872434_allCells}
p <- plotPCA(
    tmp,
...
```{r show_pca_logRaw_GSM3872434_allCells}
...
<!--
Cell-wise RLE for the `r typeNorm` counts in the `r setName` set. Each cell is represented by a box plot showing the inter-quartile range in grey, wiskers colour-coded by Sample.Name and the median as a black circle. 
-->

```{r plot_rle_logRaw_GSM3872434_allCells, eval=FALSE, include=FALSE}
p <- plotRLE(
    #tmp[,1:10],
    tmp,
    exprs_values = "logcounts_raw",
...
```{r show_rle_logRaw_GSM3872434_allCells, eval=FALSE, include=FALSE}
...
```{r comp_pca_logCpm_GSM3872434_allCells}
typeNorm <- "logCpm"
...
```{r plot_pca_logCpm_GSM3872434_allCells}
p <- plotPCA(
    logCpmPca,
    colour_by = "Sample.Name",
    size_by = "sum",
...
```{r show_pca_logCpm_GSM3872434_allCells}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r plot_rle_logCpm_GSM3872434_allCells, eval=FALSE, include=FALSE}
p <- plotRLE(
    sce,
    exprs_values = "logCpm",
    colour_by = "Sample.Name"
...
```{r show_rle_logCpm_GSM3872434_allCells, eval=FALSE, include=FALSE}
tmpFn <- sprintf("%s/%s/%s/%s_sce_nz_postQc%s_%sRle.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r comp_pca_deconv_GSM3872434_allCells}
typeNorm <- "scran"
...
```{r plot_pca_deconv_GSM3872434_allCells}
p <- plotPCA(
    scranPca,
    colour_by = "Sample.Name",
    size_by = "sum",
...
```{r show_pca_deconv_GSM3872434_allCells}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r comp_tsne_deconv_GSM3872434_allCells}
typeNorm <- "scran"
...
```{r plot_tsne_deconv_GSM3872434_allCells}
p <- plotReducedDim(
  sceDeconv,
  dimred = "TSNE_scran",
  colour_by = "Sample.Name",
...
```{r show_tsne_deconv_GSM3872434_allCells}
...
```{r  comp_umap_deconv_GSM3872434_allCells, eval=FALSE, include=FALSE}
typeNorm <- "scran"
...
```{r plot_umap_deconv_GSM3872434_allCells, eval=FALSE, include=FALSE}
p <- plotReducedDim(
  sceDeconv,
  dimred = "UMAP_scran",
  colour_by = "Sample.Name",
...
```{r show_umap_deconv_GSM3872434_allCells, eval=FALSE, include=FALSE}
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
```
...
```{r plot_rle_deconv_GSM3872434_allCells, eval=FALSE, include=FALSE}
p <- plotRLE(
    scranPca,
    exprs_values = "logcounts",
    colour_by = "Sample.Name"
...
```{r show_rle_deconv_GSM3872434_allCells, eval=FALSE, include=FALSE}
tmpFn <- sprintf("%s/%s/%s/%s_sce_nz_postQc%s_%sRle.png",
		 projDir, outDirBit, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r comp_pca_sct_GSM3872434_allCells}
typeNorm <- "sctrans"
...
```{r  plot_pca_sct_GSM3872434_allCells}
p <- plotReducedDim(
  sce,
...
```{r show_pca_sct_GSM3872434_allCells}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sPca.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r comp_tsne_sct_GSM3872434_allCells}
typeNorm <- "sctrans"
...
```{r plot_tsne_sct_GSM3872434_allCells}
p <- plotReducedDim(
  sce,
...
```{r show_tsne_sct_GSM3872434_allCells}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sTsne.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
...
```{r comp_umap_sct_GSM3872434_allCells, eval=FALSE, include=FALSE}
typeNorm <- "sctrans"
...
```{r plot_umap_sct_GSM3872434_allCells, eval=FALSE, include=FALSE}
p <- plotReducedDim(
  sce,
...
```{r show_umap_sct_GSM3872434_allCells, eval=FALSE, include=FALSE}
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
```
...
```{r plot_rle_sct_GSM3872434_allCells, cache.lazy = FALSE}
p <- plotRLE(
    sce,
...
```{r show_rle_sct_GSM3872434_allCells}
tmpFn <- sprintf("%s/%s/%s_sce_nz_postQc%s_%sRle.png",
                 dirRel, normPlotDirBit, setName, setSuf, typeNorm)
knitr::include_graphics(tmpFn, auto_pdf = TRUE)
rm(tmpFn)
