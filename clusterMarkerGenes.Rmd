...
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postDeconv%s_clust.Rds", projDir, outDirBit, setName, setSuf)
print(tmpFn)
...
umap1 <- plotUMAP(sce, colour_by=mrkSet[geneInd,"ensembl_gene_id"]) + fontsize
tsne1b <- plotTSNE(mnn.out, colour_by=mrkSet[geneInd,"Symbol"], by_exprs_values = "reconstructed") + fontsize
umap1b <- plotUMAP(mnn.out, colour_by=mrkSet[geneInd,"Symbol"], by_exprs_values = "reconstructed") + fontsize
...
```{r, eval=FALSE}
markersWoBatch.up <- findMarkers(sce, groups=sce$clusterStg, direction="up")
```
...
markersWoBatch.up3 <- findMarkers(sce, groups=sce$clusterStg, pval.type="all", direction="up")
```
...
```{r}
markersWoBatch.up4 <- findMarkers(sce, groups=sce$clusterStg, pval.type="some", direction="up")
```
...
```{r}
markersWoBatch.binom <- findMarkers(sce, test="binom", direction="up", groups=sce$clusterStg)
```
...
```{r}
tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postDeconv%s_clustMarkCombi.Rds",
