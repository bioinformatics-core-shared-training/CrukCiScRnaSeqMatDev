...
# Clustering with `r sprintf("'%s'", splSetToGet)` and '`r setSuf`'
...
```{r plot_tsne_hierar_facet, fig.width=10, fig.height=10}
...
# plot:
plot(sil, main = paste(length(unique(hc.clusters)), "clusters"), 
...
```{r plot_tSNE_kmeans_k6_split, fig.width=10, fig.height=10}
...
The plot above suggests that k would be best chosen between 6 and 9.
...
gaps <- cluster::clusGap(
  ...
  FUNcluster = kmeans,
  K.max = 20,
  nstart = 5, # for speed here but should use higher
  B = 10 # for speed here but should use higher
)
...
# write NbClust object to file:
#tmpFn <- sprintf("%s/%s/Robjects/%s_sce_nz_postDeconv%s_1kCellK2to20NbClust.Rds", projDir, outDirBit, setName, setSuf) # too slow with 1000 cells
...
Heatmap display of the contingency table for the graph-based clusters and the
k-means clusters used to build the network (color-code by size, i.e. number of cells).
...
pheatmap::pheatmap(tmpTab, show_colnames = FALSE)
```
...
```{r cluster_louvain}
cl <- igraph::cluster_louvain(snn.gr)$membership
cl <- factor(cl)
# store membership
sce$louvain <- cl
# show cluster sizes:
...
```{r show_louvain_tsne_2, fig.width=10, fig.height=10}
...
We now compute modularity to assess clusters quality (the division of cells into several groups, 'clusters', is compared to that in an equivalent random network with no structure). The overall modularity is `r round(igraph::modularity(cluster.out),2)`. <!-- cluster.out is walktrap not leiden -->
...
mod.out <- bluster::pairwiseModularity(snn.gr, wt.clusters, get.weights=TRUE)
...
```{r show_leiden_tsne_2, fig.width=10, fig.height=10}
# facet by sample group:
p + facet_wrap(~ sce$source_name)
```
...
The contingency table below shows the concordance between the k-means (columns) and walk-trap (rows) clustering methods:
...
rownames(sce) <- scater::uniquifyFeatureNames(
  ...
  rowData(sce)$Symbol)
plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "MS4A1") +
  ggtitle("B cells: MS4A1")
plotTSNE(sce, by_exprs_values = "reconstructed", colour_by = "CD79A") +
...
```{r}
# Naive CD4+ T
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[1:2]) + ggtitle("Naive CD4+ T cells")

# Memory CD4+
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[c(1,3)]) + ggtitle("Memory CD4+ T cells")

# B cells
##plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[6]) + ggtitle("B cells")

# CD8+ T
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[7]) + ggtitle("CD8+ T cells")

# NK
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[10:11]) + ggtitle("NK")

# CD14+ Mono
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[4:5]) + ggtitle("CD14+ Monocytes")

# FCGR3A+ Mono
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[8:9]) + ggtitle("FCGR3A+ Monocytes")

# DC
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[12:13]) + ggtitle("Dendritic Cells")

# Platelet
plotExpression(sce, exprs_values = "reconstructed", x=cluToUse, colour_by=cluToUse, features=markGenes[14]) + ggtitle("Platelet")
```
